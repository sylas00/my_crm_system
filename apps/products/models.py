...
from upload.models import ProductDocumentModel
...
class ProductModel(BaseModel):
    ...
    platform = models.ForeignKey(ProductPlatformModel, on_delete=models.CASCADE, blank=True, verbose_name='产品平台')
    type = models.ForeignKey(ProductTypeModel, on_delete=models.CASCADE, verbose_name='产品类型')
    giveaway = models.ManyToManyField(GiveawayModel, blank=True, verbose_name='赠品')
    contract_file = models.ForeignKey(ProductDocumentModel, on_delete=models.CASCADE, blank=True, verbose_name='合同附件')
    # 收费模式
    ...
    minimum_price_month = models.DecimalField(max_digits=20, decimal_places=2, null=True, blank=True,
                                              verbose_name='最低月度报价')
    # 收费模式2 服务次数
    service_time = models.ManyToManyField(ServiceTimeModel, blank=True, verbose_name='服务次数套餐')
